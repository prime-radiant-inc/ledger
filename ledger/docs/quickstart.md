...
2. **Orientation.** `ls` lists ledgers; `show` is the full render; `status
   [key]` is the spine. `status <key> --ledger <slug>` composes both, and
   `--ledger` is *required* once more than one ledger is open. Empty
   output announces itself; it never just hangs.
...
5. **One body source per note.** `-m`, `--from-file <path>`, or stdin —
   ...
   `carry-forward`, `handoff`, `gotcha`, `postmortem`. Read back with
   `notes [-k kind] [--key k] [--latest | -n N]`.
...
ledger note -k handoff --key qs-demo-task2 --from-file handoff.md
